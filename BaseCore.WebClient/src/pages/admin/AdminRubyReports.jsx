...
function StatusPill({ status, detail }) {
  ...
  return (
    ...
    >
      <i
        className={`fa-solid ${isOk ? "fa-circle-check" : "fa-triangle-exclamation"}`}
      />
      {status || "unknown"}
    </span>
  );
...
export default function AdminRubyReports() {
  ...
  const loadReports = async ({ quiet = false } = {}) => {
    ...
    const [healthResult, readyResult, catalogResult, summaryResult] =
    ...
    try {
      ...
      setError(
        await extractErrorMessage(
          err,
          "Không xuất được file CSV từ RubyReportService.",
        ),
      );
    } finally {
      setExporting("");
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-warmink-2">
            RubyReportService xuất báo cáo CSV
          </p>
        </div>
        <button
          type="button"
        ...
        >
          {refreshing ? (
            <span className="h-4 w-4 rounded-full border-2 border-warmborder-2 border-t-transparent animate-spin" />
          ) : (
        ...
        <section className="card p-4">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end">
            <label className="block">
              <span className="mb-1 block text-sm text-warmink-2">
                Số dòng tối đa
              </span>
              <input
                type="number"
          ...
          {lastDownload && (
            <p className="mt-3 text-sm text-emerald-700">
              ...
              {lastDownload.filename} · {lastDownload.limit} dòng ·{" "}
              {formatDateTime(lastDownload.at)}
            </p>
          )}
        </section>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatTile
          label="Campaign"
          value={formatNumber(totals.supportCampaigns)}
          icon="fa-bullhorn"
          tone="text-amber-600"
        />
        <StatTile
          label="Donation"
          value={formatNumber(totals.donations)}
          icon="fa-hand-holding-heart"
          tone="text-emerald-600"
        />
        <StatTile
          label="Đã xác nhận"
          value={formatMoney(totals.confirmedDonationAmount)}
          icon="fa-sack-dollar"
